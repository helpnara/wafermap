...
| v1.3 | 2026-08-18 | **용어사전을 독립 메뉴로 격상**(§5-A) — 비전문가 기준 설명·비유·산업적 의미. **분석 해석 가이드**(`08_interpretation_guide.md`) 신설 — 각 단계의 '왜 중요한가 / 어떻게 읽나 / 무엇을 조치하나 / 함정' |
...
| **`8_도움말_학습`** | **코드 학습(§5)** | 모듈별 개념 설명, 실제 소스코드 하이라이트, 설계 노트, 자가진단. **모바일 전용 카드 레이아웃(§4A.6)** |
| **`9_용어사전`** | **용어 학습(신규, §5-A)** | 반도체·공정·통계·ML 용어 98개. 분류별 탐색, 검색, 비전문가용 설명·비유, **왜 중요한가**, 관련어 링크 |
...
### 4.3 배포 — ❌ 범위에서 제외 (2026-08-20)

**클라우드 배포는 이 과제의 범위가 아니다.** 제약으로 진행이 어려워 사용자가 제외를
결정했다. 아래 표는 원래 계획이며 **기록으로만** 남긴다.

<details>
<summary>원래 배포 계획 (실행하지 않음)</summary>
...
**남는 실행 경로는 로컬뿐이다.** 설치와 실행은 `docs/07_local_setup.md`에 있다.

**모바일은 어떻게 되나**: §4-A의 반응형 설계와 §4A.6 모바일 학습 모드는 **그대로
유효하다.** 원래 "공유 URL로 휴대폰 열람"을 주 경로로 잡았지만, 로컬 실행에서도
같은 네트워크의 휴대폰에서 `http://<PC의 IP>:8501` 로 열 수 있다.
접기형 아이콘 메뉴와 진도 저장이 필요한 이유는 배포 방식과 무관하다.

**이 결정으로 사라진 제약**: CNN 파라미터 상한(~20만)의 원래 근거는 클라우드 메모리
한도였다. 지금은 **LightGBM과의 공정한 대조**가 근거다 — 비슷한 규모로 맞춰야
"딥러닝이 더 낫다/아니다"가 용량 차이가 아니라 방식 차이의 결과가 된다.
...
이 프로젝트는 **두 개의 낯선 세계를 겹쳐 놓았다.** 반도체 공정(EDS, FDC, CMP, 챔버…)과
통계·머신러닝(macro-F1, BH-FDR, SHAP…)이다.

어느 한쪽에 익숙한 사람도 반대쪽은 처음일 수 있고, **포트폴리오를 보는 사람은 양쪽 다
모를 수도 있다.** 용어가 막히면 아무리 좋은 분석도 전달되지 않는다.

학습노트(§5) 안의 각주로 처리하기에는 분량과 활용도가 커서 독립 메뉴로 둔다.
용어는 특정 모듈에 종속되지 않고 **전 페이지에서 참조**되기 때문이기도 하다.
...
| `plain` | **비전문가용.** 전문용어를 쓰지 않고 일상의 비유로 설명 |
| `detail` | 더 정확한 설명 (선택, 접어 둠) |
| `why_matters` | **왜 반도체 현업에서 중요한가.** 돈·시간·품질과의 연결 |
| `aliases` | 검색용 별칭 — 영문·약어·한글 표기 |
| `related` | 관련 용어 (사전에 반드시 존재해야 함) |
| `used_in` | 프로젝트에서 등장하는 파일 경로 |

`why_matters`가 핵심이다. 학습이 목적이라면 정의만으로는 부족하다.
**"그래서 이걸 왜 배우지?"** 에 답해야 기억에 남는다.

### 5A.3 규모 (v1.3 기준)

| 분류 | 개수 | 예시 |
|---|---:|---|
| 제품 | 9 | 웨이퍼, die, lot, 수율, DRAM |
| 테스트 | 6 | EDS, wafer map, Bin, 리페어 |
| 공정 | 11 | 포토, 식각, 증착, CMP, 세정 |
| 설비 | 22 | FDC, 챔버, PM, 포커스 링, 파라미터 |
| 불량패턴 | 10 | Center, Edge-Ring, Scratch … 9종 + 총론 |
| 통계 | 15 | SPC, 커미널리티 분석, 교락, BH-FDR |
| 머신러닝 | 20 | macro-F1, 데이터 누출, SHAP, 지름길 학습 |
| 데이터 | 5 | WM-811K, 합성 데이터, 정답지 |
| **합계** | **98** | |

### 5A.4 품질 보증

용어사전은 '읽는 자료'라 버그가 나도 프로그램이 죽지 않는다. **대신 조용히 잘못된 것을
가르친다.** 그래서 아래를 테스트로 강제한다(`tests/test_glossary.py`, 80건).

- 관련어 링크가 실제로 존재하는가 (깨진 링크 = 클릭했더니 없는 용어)
- 별칭이 다른 용어와 충돌하지 않는가
- 프로젝트 핵심 어휘 40여 개가 빠지지 않았는가
- 중요 용어 19개가 `why_matters`를 갖췄는가
- `used_in` 경로가 실재하는가

> 실제로 이 테스트가 결함을 잡아냈다. 식각 챔버의 소모품 부품(edge ring)과
> 불량 패턴(Edge-Ring)이 **이름이 같아** 검색이 엉켰다. 현업에서도 혼동되는 지점이라,
> 부품을 `포커스 링`으로 표기하고 **그 혼동 자체를 설명에 담았다.**

### 5A.5 다른 화면과의 연결

- **자동 링크**: `find_in_text()` 가 본문에서 용어를 찾아낸다. 학습노트·화면 설명에
  나온 전문용어에 사전 링크를 자동으로 걸어, 읽다 막히면 바로 뜻을 볼 수 있게 한다.
- **툴팁**: 표·차트의 항목명에 `short` 를 툴팁으로 붙인다.
- **터미널 조회**: `python scripts/glossary.py <용어>` — 앱을 띄우지 않고도 찾아본다.

### 5A.6 의존성 제약

`learning/glossary.py` 는 `data/`·`models/`·pandas·numpy를 **import하지 않는다.**
용어사전은 데이터 로딩 없이 즉시 떠야 모바일에서 '틈틈이 공부'가 성립한다(§4A.7).
이 제약도 테스트로 강제한다.
...
**분석 도구를 만드는 것과 그 결과를 읽어 내는 것은 다른 능력이다.**
실무에서는 후자가 더 어렵고 더 중요하다.

"macro-F1 0.974"라는 숫자를 보고 ① 이게 좋은 건지 ② 왜 정확도가 아니라 이걸 보는지
③ 무엇을 더 확인해야 하는지를 아는 것이 실제 역량이다.

이 프로젝트는 **학습이 주목적**이므로, 각 분석 단계마다 아래 네 가지를 명시한다.

| 항목 | 내용 |
|---|---|
| **왜 중요한가** | 이 분석이 반도체 현업에서 갖는 의미 |
| **어떻게 읽나** | 산출된 숫자·그래프의 해석 방법 |
| **무엇을 조치하나** | 결과가 이러면 무엇을 하는가 |
| **함정** | 잘못 읽기 쉬운 지점 |

### 5B.2 산출물

`docs/08_interpretation_guide.md` — 6개 분석 단계 전체를 다룬다.

핵심 메시지는 **"매 단계가 다음 단계의 범위를 좁힌다"** 이다.

```
웨이퍼 맵 → 패턴 분류 → SPC → 커미널리티 → SHAP → 개선안
 무엇이     어떤 종류    언제부터  어느 설비    무엇이   어떻게
```

반도체 공장에는 수백 개 공정과 수십 대 장비가 있어 처음부터 전부 뒤질 수 없다.
깔때기처럼 좁혀 가는 이 **사고 과정 자체**가 이 프로젝트가 흉내 내려는 대상이다.

### 5B.3 앱 반영 방식

- 각 분석 페이지 상단에 접힌 `st.expander("💡 이 분석이 왜 중요한가요?")`
- 결과 표·차트 아래에 `📖 이 숫자 읽는 법` 안내
- 함정에 해당하는 항목에는 ⚠️ 배지 (예: 보정 전 p-value를 보여줄 때)

기본은 접힘 상태라 포트폴리오 열람 시 방해되지 않고, 학습할 때만 펼친다.
...
│   └─ glossary.py                   # 📖 용어사전 (§5-A, 데이터 계층 의존 금지)
├─ src/wafermap/
...
│   ├─ learning/                     # 학습 지원 (§5, §5-A) — data/·models/ import 금지(§4A.7)
│   │   ├─ loader.py                 # 학습노트 파싱, 소스코드 발췌
│   │   └─ glossary.py               # 용어 98개 + 검색·분류·자동링크 API
│   ├─ ui/                           # 반응형·네비게이션 공통 (§4-A)
...
│   ├─ 08_interpretation_guide.md    # 분석 해석 가이드 (§5-B)
...
| **M6** | Streamlit 통합 | 10개 화면(개요+9) 동작, 전역 필터, 캐시 최적화, **반응형 3-모드 네비게이션 + 모바일 레이아웃 검증(§4-A)**, 용어사전 자동 링크 연동(§5A.5) |
| **M7** | 검증·문서·로컬전환 | 테스트 통과, README·결과 문서, `06_local_validation.md` 작성, 로컬 실데이터 재검증 절차 확인 (**배포는 범위 제외 — §4.3**) |
...
| **용어사전 커버리지** | 프로젝트 핵심 어휘 100% 수록, 중요 용어는 산업적 의미(`why_matters`) 포함 |
| **해석 가이드** | 6개 분석 단계 전체에 '왜 중요한가/어떻게 읽나/조치/함정' 기술 |
...
| 용어사전이 코드와 어긋남(깨진 링크, 누락) | 잘못된 내용을 학습 | 관련어 무결성·핵심 어휘 커버리지·경로 실재 여부를 테스트로 강제(§5A.4) |
| 같은 이름의 부품과 불량 패턴 혼동 | 검색·학습 혼선 | 부품을 `포커스 링`으로 표기하고 혼동 자체를 설명에 명시(§5A.4) |
