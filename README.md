...
| 단계 | 질문 | 방법 | 측정된 성능 |
|---|---|---|---|
| M2 | 어떤 패턴인가 | 기하 피처 + LightGBM | macro-F1 **0.974** |
| M3 | 언제 · 어느 설비인가 | p-chart SPC + 커미널리티 | 원인 챔버 Top-1 **100%** |
| M4 | 그 설비의 무엇이 | LightGBM + SHAP | 원인 파라미터 Top-3 **100%** |
| M5 | 무엇을 바꿀 것인가 | 권고 구간 + 반사실 시뮬레이션 | 실행 가능성 제약 반영 |

성능 수치는 시뮬레이터가 심어 둔 **정답지와 대조한 채점** 결과입니다.
