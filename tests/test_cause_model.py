...
def test_parameter_columns_excludes_identifiers(make_fdc):
...
def test_parameter_columns_drops_constant(make_fdc):
...
def test_min_max_excluded_to_avoid_split_importance(make_fdc):
...
def test_finds_injected_culprit_parameter(make_fdc):
    """인공적으로 이동시킨 파라미터가 SHAP 1위여야 한다."""
    fdc, case, control = make_fdc(culprit="chamber_pressure", shift=3.0, seed=1)
...
def test_dependence_curve_is_monotonic_for_shifted_param(make_fdc):
    """이동시킨 파라미터는 값이 커질수록 기여도가 올라가야 한다."""
    fdc, case, control = make_fdc(culprit="chamber_pressure", shift=3.0, seed=6)
    result = cause_model.fit(fdc, "P020", "P", case, control)
...
def test_dependence_curve_rejects_unknown_feature(make_fdc):
    fdc, case, control = make_fdc(seed=7)
    result = cause_model.fit(fdc, "P020", "P", case, control)
    with pytest.raises(KeyError):
        cause_model.dependence_curve(result, "not_a_param")


def test_recommend_spec_within_observed_range(make_fdc):
    """권고 구간이 관측 범위를 벗어나면 안 된다 (외삽 방지)."""
    fdc, case, control = make_fdc(culprit="chamber_pressure", shift=3.0, seed=8)
    result = cause_model.fit(fdc, "P020", "P", case, control)
...
def test_explain_wafer_returns_top_contributions(make_fdc):
    fdc, case, control = make_fdc(seed=9)
    result = cause_model.fit(fdc, "P020", "P", case, control)
...
def test_explain_wafer_rejects_bad_index(make_fdc):
    fdc, case, control = make_fdc(seed=10)
    result = cause_model.fit(fdc, "P020", "P", case, control)
...
def test_compare_distributions_finds_shifted_param(make_fdc):
    """분포 비교가 이동된 파라미터를 잡아내야 한다 (모델 없이)."""
    fdc, case, control = make_fdc(culprit="o2_flow", shift=3.0, seed=11)
...
def test_build_evidence_combines_shap_and_distribution(make_fdc):
    """SHAP 결과와 분포 증거가 함께 담겨야 한다."""
    fdc, case, control = make_fdc(culprit="chamber_pressure", shift=3.0, seed=12)
    result = cause_model.fit(fdc, "P020", "P", case, control)
